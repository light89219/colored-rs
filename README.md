...
[![Crates.io](https://img.shields.io/crates/v/colored-rs.svg?maxAge=2592000)](https://crates.io/crates/colored-rs) [![Crates.io](https://img.shields.io/crates/l/colored-rs.svg?maxAge=2592000)](https://github.com/colored-rs/colored-rs/blob/master/LICENSE)
...
```rust
    extern crate colored_rs; // not needed in Rust 2018+
...
Colored-rs has support for truecolors where you can specify any arbitrary rgb value.
...
```toml
[features]
# this effectively enable the feature `no-color` of colored-rs when testing with
# `cargo test --features dumb_terminal`
dumb_terminal = ["colored-rs/no-color"]
```

You can have even finer control by using the
`colored_rs::control::set_override` method.
